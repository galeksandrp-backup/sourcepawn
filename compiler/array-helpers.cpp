...
ArraySizeResolver::ResolveRank(int rank, Expr* init)
{
    ...
    SetRankSize(expr, rank, expr->exprs().length());
...
{
    if (Expr* init = decl_->init_rhs()) {
        if (decl_->vclass() == sARGUMENT)
            return CheckArgument(init);
        if (!ValidateRank(0, init))
            return false;
        return true;
    }

    for (int i = 0; i < type_.numdim; i++) {
        if (!type_.dim[i] && decl_->vclass() != sARGUMENT) {
...
FixedArrayValidator::ValidateRank(int rank, Expr* init)
{
    if (rank != type_.numdim - 1) {
        ...
        if ((cell)array->exprs().length() != type_.dim[rank]) {
            error(init->pos(), 47);
            return false;
        }

        if (!AddCells(array->exprs().length()))
            return false;
    ...
    ArrayExpr* array = init->AsArrayExpr();
    if (!array || (rank_size && rank_size < (cell)array->exprs().length())) {
        error(init->pos(), 47);
        return false;
    }
    ...
    cell ncells = rank_size ? rank_size : array->exprs().length();
    if (!AddCells(ncells))
        return false;
    ...
    if (array->ellipses()) {
        ...
        if (prev1.isValid() && prev2.isValid() && type_.tag) {
            ...
            error(array->exprs().back()->pos(), 70, type_to_name(type_.tag));
            return false;
        }
        if (!rank_size || rank_size == (cell)array->exprs().length()) {
...
CheckArrayDeclaration(VarDecl* decl)
{
    ...
    if (gTypes.find(type.semantic_tag())->isEnumStruct())
        expected_dims--;
    if (expected_dims != ctor->exprs().length()) {
        error(19, (int)expected_dims, (int)ctor->exprs().length());
        return false;
    }
...
class ArrayEmitter final
{
  public:
    ...
    size_t data_size() const {
        return data_.length() + pending_zeroes_;
    }
...
ArrayEmitter::Emit()
{
    ...
    for (size_t i = 0; i < iv_.length(); i++) {
        if (!(iv_[i] & kDataFlag))
            continue;
        iv_[i] &= ~kDataFlag;
        iv_[i] += ((cell)iv_.length() * sizeof(cell));
...
ArrayEmitter::Emit(int rank, Expr* init)
{
    ...
    size_t rank_size = type_.dim[rank];
    if (type_.tag == pc_tag_string)
...
ArrayEmitter::add_data(cell value)
{
    ...
    if (pending_zeroes_) {
        data_.resize(data_.length() + pending_zeroes_);
        pending_zeroes_ = 0;
    }
    data_.append(value);
}
...
BuildArrayInitializer(const typeinfo_t& type, Expr* init, ArrayData* array)
{
    ...
    array->iv = ke::Move(emitter.iv());
    array->data = ke::Move(emitter.data());
