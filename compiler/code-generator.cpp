...
VarDecl::EmitGlobal()
{
    ...
    if (sym_->ident == iVARIABLE) {
        ...
        gDataQueue.Add(ke::Move(array.iv));
        gDataQueue.Add(ke::Move(array.data));
...
VarDecl::EmitLocal()
{
    if (sym_->ident == iVARIABLE) {
        ...
        cell iv_size = array.iv.length();
        cell data_size = array.data.length() + array.zeroes;
        ...
        if (!array.zeroes) {
            ...
            for (size_t i = 1; i < array.data.length(); i++) {
...
{
    int numdim = 0;
    for (size_t i = 0; i < exprs_.length(); i++) {
        Expr* expr = exprs_[i];
        expr->Emit();

        if (i == exprs_.length() - 1 && tag_ == pc_tag_string)
            stradjust(sPRI);
