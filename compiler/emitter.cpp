...
DataQueue::Compact()
{
    ...
    if (current().zeroes < 16) {
        current().cells.resize(current().cells.length() + current().zeroes);
        current().zeroes = 0;
        return true;
    }
    return false;
...
void
DataQueue::Add(ke::Vector<cell>&& cells)
{
    if (cells.empty())
        return;
...
class DataEmitter final
{
  public:
    ...
    void Dump(const ke::Vector<cell>& vals);
...
void
DataEmitter::Dump(const ke::Vector<cell>& vals)
{
...
emit_default_array(arginfo* arg)
{
    ...
    if (!def->val) {
        cell iv_size = (cell)def->array->iv.length();
        cell data_size = (cell)def->array->data.length();
        def->val = ke::Some(gDataQueue.dat_address());
        gDataQueue.Add(ke::Move(def->array->iv));
        gDataQueue.Add(ke::Move(def->array->data));
        gDataQueue.AddZeroes(def->array->zeroes);

        // Save the iv and data sizes back. Note that the move into DQ should
        // have cleared the array.
        def->array->iv.append(iv_size);
        def->array->data.append(data_size);
    }
    ...
    if (arg->is_const) {
        ...
        cell iv_size = def->array->iv[0];
        cell data_size = def->array->data[0];
