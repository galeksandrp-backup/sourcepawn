...
CompilerBase::emit()
{
  ...
  size_t rt_len = strlen(rt_->Name());
  size_t len = rt_len + strlen("::") + strlen(function_name);

  debug_name_ = std::make_unique<char[]>(len + 1);
  strcpy(debug_name_.get(), rt_->Name());
  strcpy(debug_name_.get() + rt_len, "::");
  strcpy(debug_name_.get() + rt_len + 2, function_name);
  ...
  CodeChunk code = LinkCode(env_, masm, debug_name_.get(), debug_map);
  if (!code.address()) {
